...
export async function getServerSideProps(ctx){
    ...
    const reqPengawas = await fetch(`${process.env.PUBLIC_URL}/api/pengawas`, {
        method: 'GET',
        headers: {
    ...
    const resPengawas = await reqPengawas.json()
    const data = resPengawas.code === 200 ? resPengawas.data : []
    return {
        props: {
            data
        }
...
const Pengawas = ({ data }) => {
    ...
    const handlerSubmit = async(e) => {
        e.preventDefault()
        const reqTambahPengawas = await fetch('/api/pengawas', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        const resTambahPengawas = await reqTambahPengawas.json()
        if(resTambahPengawas.code === 200) return alert(resTambahPengawas.message),Router.push('/pengawas')
        return alert(resTambahPengawas.message)
    }

    const handlerDelete = async(e, bpo) => {
        e.preventDefault()
        const status = confirm(`Yakin ingin menghapus data pengawas dengan BPO : ${bpo} ?`)
        const reqDeletePengawas = await fetch(`/api/pengawas/${bpo}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        const resDeletePengawas = await reqDeletePengawas.json()
        if(resDeletePengawas.code !== 200){
            alert(resDeletePengawas.message)
        }else{
            alert(resDeletePengawas.message)
            ...
            <div className="modal-dialog">
                ...
                <div className="modal-body">
                    ...
                    <div className="mb-3 row">
                        <label htmlFor="bpo" className="col-sm-3 col-form-label">Kode Tanda Pengawas</label>
