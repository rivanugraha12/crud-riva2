...
export async function getServerSideProps(ctx){
    ...
    const reqAnggota = await fetch(`${process.env.PUBLIC_URL}/api/anggota`, {
        method: 'GET',
        headers: {
...
const Anggota = ({ data }) => {
    ...
    const handlerSubmit = async(e) => {
        e.preventDefault()
        const reqTambahAnggota = await fetch('/api/anggota', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if(resTambahAnggota.code === 200) return alert(resTambahAnggota.message),Router.push('/anggota')
        return alert(resTambahAnggota.message)
    }

    const handlerDelete = async(e, kta) => {
        e.preventDefault()
        const status = confirm(`Yakin ingin menghapus data anggota dengan KTA : ${kta} ?`)
        const reqDeleteAnggota = await fetch(`/api/anggota/${kta}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        <>
            ...
            <div className="modal-dialog">
                ...
                <div className="modal-header">
                    <h1 className="modal-title fs-5" id="exampleModalLabel">Tambah Data Anggota</h1>
                    <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div className="modal-body">
                    
                    <div className="mb-3 row">
                        <label htmlFor="kta" className="col-sm-3 col-form-label">Kartu Tanda Anggota</label>
                        <div className="col-sm-9">
                            <input type="text" className="form-control" id="kta" name='kta' onChange={e => handlerChange(e)} value={fields.kta}/>
                        </div>
                    </div>
                            ...
                            data.map((element, index) => {
                                    return  <tr key={index}>
                                                ...
                                                <td className='d-flex flex-row gap-1'>
                                                    <Link href={`/anggota/${element.kta}`} className='btn btn-warning btn-sm'>Edit</Link>
                                                    <button className='btn btn-danger btn-sm' onClick={e => handlerDelete(e, element.kta)}>Hapus</button>
                                                </td>
                                            </tr>
                                }):
                            <tr>
